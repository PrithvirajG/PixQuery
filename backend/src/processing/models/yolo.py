...
class YoloModel(ModelInterface):
    ...
    def detect(self, image: Image.Image, write_image: bool = False) -> list | None:
        try:
            ...
            if write_image:
                self.write_image_with_detections(image, detections, "src/processing/processed_media_dump/yolo_out.jpg")
    ...
    def write_image_with_detections(self, image: Image.Image, detections: list, save_path: str):
        ...
        try:
            font = ImageFont.truetype("arial.ttf", 14)  # Fallback handled below
        except:

"""A pure-Python stand-in for a pymongo ``Database``, used by tests.

Generic and collection-agnostic: nothing here knows about PixQuery's schema. Any
repository built as ``SomeRepository(database)`` — real pymongo ``Database`` or
:class:`FakeDatabase` — works identically either way, since both expose the same
``find``/``find_one``/``update_one``/... surface.
"""
...
class FakeCollection:
    ...
    def find(self, query):
        return FakeCursor([deepcopy(doc) for doc in self.docs if _matches(doc, query)])

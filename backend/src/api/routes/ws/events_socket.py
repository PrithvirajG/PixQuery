...
the detail endpoint for the substance. See ``src/domain_events.py`` for why the
payloads are deliberately thin.
...
async def get_subscriber():
    ...
    async with _subscriber_lock:
        if _subscriber is None:
            from src.consumer.events import EventConsumer
...
class _AccessGate:
    ...
    def __init__(self, workspaces: WorkspaceDefinitionsRepository, user_id: str):
    ...
    def allows(self, workspace_id: str | None) -> bool:
        ...
        if now - self._checked_at > _ACCESS_REFRESH_SECONDS:
            try:
                self._ids = set(
                    accessible_workspace_ids(self._workspaces, self._user_id)
                )
            except Exception:
...
async def events_socket(websocket: WebSocket, token: str | None = None):
    from src.api.dependencies import get_users_repository, get_workspace_definitions_repository

    users = get_users_repository()
    workspaces = get_workspace_definitions_repository()
    user = authenticate_from_token(token, users)
    if not user:
    ...
    gate = _AccessGate(workspaces, user["_id"])
